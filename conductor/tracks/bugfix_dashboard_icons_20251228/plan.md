...
## Plan
- [ ] Task: Audit Dashboard Components [Podium.tsx, MissionMeter.tsx, StudentLeaderboard.tsx]
    - [ ] Check how icons are rendered in the headers of these components.
    - [ ] Identify if styles are applied to the icon directly or a wrapper.
    - [ ] Ensure wrappers have `flex items-center justify-center`.
- [ ] Task: Fix Alignment
    - [ ] Apply the same fix (centering wrapper) to Dashboard components as was done for Admin.
    - [ ] If "not in a circle" persists, check if `w-` / `h-` classes on the icon match the font-size logic in `Icons.tsx`.
