...
const PointsManager = React.lazy(() => import('./PointsManager').then(m => ({ default: m.PointsManager })));
const UsersManager = React.lazy(() => import('./UsersManager').then(m => ({ default: m.UsersManager })));
const SchoolSettings = React.lazy(() => import('./SchoolSettings').then(m => ({ default: m.SchoolSettings })));
const ClassesManager = React.lazy(() => import('./ClassesManager').then(m => ({ default: m.ClassesManager })));
const ActionLogPanel = React.lazy(() => import('./ActionLogPanel').then(m => ({ default: m.ActionLogPanel })));
const MessagesManager = React.lazy(() => import('./MessagesManager').then(m => ({ default: m.MessagesManager })));
const MyClassStatus = React.lazy(() => import('./MyClassStatus').then(m => ({ default: m.MyClassStatus })));
const DataManagement = React.lazy(() => import('./DataManagement').then(m => ({ default: m.DataManagement })));
const GoalsManagement = React.lazy(() => import('./GoalsManagement').then(m => ({ default: m.GoalsManagement })));
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const activeTab = useMemo(() => {
    ...
    return isAdmin ? 'school' : 'points';
  }, [activeTabFromUrl, initialTab, isAdmin]);
  ...
  return (
    <GradientBackground primaryColor={settings.primary_color} secondaryColor={settings.secondary_color} brightness={settings.background_brightness}>
      ...
      <div className="flex flex-col h-full w-full overflow-hidden relative z-10 p-2 md:p-4">

        <MotionDiv
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex-1 w-full max-w-[1920px] mx-auto bg-[var(--bg-card)]/40 backdrop-blur-3xl border border-[var(--border-main)] rounded-[2rem] md:rounded-[2.5rem] shadow-[var(--shadow-main)] flex flex-col md:flex-row overflow-hidden relative"
        >
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-[var(--bg-card)]/5 overflow-hidden">
              ...
              <Suspense fallback={<LoadingTab />}>
                <AnimatePresence mode='wait'>
                  <MotionDiv key={activeTab} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.2 }} className="min-h-full">
                    ...
                    {activeTab === 'logs' && (<ActionLogPanel logs={logs} onLoadMore={loadMoreLogs} onDelete={(id) => deleteLog(id)} onUpdate={(id, description, points) => updateLog({ id, description, points })} currentUser={user} settings={settings} />)}
                  </MotionDiv>
                </AnimatePresence>
              </Suspense>
            </main>
