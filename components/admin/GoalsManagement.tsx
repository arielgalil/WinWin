...
import { TargetIcon, SaveIcon, CheckIcon, RefreshIcon, AlertIcon } from '../ui/Icons';
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-12 pb-20 px-4">
            <div className="flex items-center gap-4 border-b border-white/10 pb-6">
                <div className="w-14 h-14 bg-gradient-to-br from-orange-500 to-red-600 rounded-[var(--radius-main)] flex items-center justify-center shadow-lg shadow-orange-900/20">
            ...
            <section className="space-y-6 pt-8 border-t border-white/10">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/30 p-3 rounded-[var(--radius-main)] flex items-center gap-3 rtl:mr-4 ltr:ml-4">
...
const ClassTargetCard = React.memo(({ cls, onSave }: { cls: ClassRoom, onSave: (id: string, val: number) => Promise<void> }) => {
    ...
    return (
        <div className={`relative p-5 rounded-[var(--radius-main)] border transition-all overflow-hidden flex flex-col gap-4 ${isCompleted ? 'bg-yellow-900/20 border-yellow-500/30' : 'bg-white/5 border-white/10'}`}>
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center">
                {!hasTarget ? (
                    <div className="text-slate-500 text-[10px] font-bold bg-white/5 py-2 rounded-[var(--radius-main)] border border-dashed border-white/5 text-center">{t('no_target_set')}</div>
                ) : (
            ...
            <div className="mt-auto pt-3 border-t border-white/5">
                <div className="flex items-center gap-2 bg-black/20 p-1.5 rounded-[var(--radius-main)] border border-white/5">
                    ...
                    <button onClick={handleSave} disabled={isSaving} className={`w-8 h-8 rounded-[var(--radius-main)] flex items-center justify-center ${isSaved ? 'bg-green-600' : 'bg-blue-600 hover:bg-blue-500'} text-white transition-colors`}>
