...
export const useCompetitionMutations = (campaignId: string | undefined) => {
  ...
  const deleteLog = useCallback(async (id: string) => {
    const { data: log, error: fetchErr } = await supabase.from("action_logs")
      .select("is_cancelled").eq("id", id).single();
    if (fetchErr) {
      console.error("Error fetching log state:", fetchErr);
      return;
    }
  ...
  const updateLog = useCallback(
    async (
      ...
      const { error } = await supabase.from("action_logs").update({
        description,
        points,
      }).eq("id", id);
  ...
  const updateClassTarget = useCallback(
    async (classId: string, targetScore: number) => {
  ...
  const updateSettingsGoals = useCallback(
    async (goals: CompetitionGoal[], gridSize: number) => {
      if (!campaignId) return;
      const { error } = await supabase.from("app_settings").update({
        goals_config: goals,
  ...
  const updateIrisPattern = useCallback(
    async (
      pattern: { cx: number; cy: number; weight: number; delay: number }[],
    ) => {
      if (!campaignId) return;
      const { error } = await supabase.from("app_settings").update({
        iris_pattern: pattern,
      }).eq("campaign_id", campaignId);
      if (error) console.error("Iris pattern update failed", error);
      // Don't invalidate - this is a one-time setup that shouldn't trigger re-fetches
    },
    [campaignId],
  );
