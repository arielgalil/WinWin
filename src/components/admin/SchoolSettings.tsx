...
export const SchoolSettings: React.FC<SchoolSettingsProps> = (
    ...
    const handleSaveSettings = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                background_brightness: Math.round(formData.background_brightness ?? 50),
                ...
                background_music_volume: Math.round(formData.background_music_volume ?? 50),
    ...
    const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const fileName =
                `logo_${settings.campaign_id}_${Date.now()}.${fileExt}`;
    ...
    const removePreset = (index: number) => {
        ...
        openConfirmation({
            ...
            message: t("delete_preset_confirm", {
                label: presetToRemove?.label || "",
            }),
            isDanger: true,
            onConfirm: () => {
    ...
    return (
        <div className="space-y-[var(--admin-section-gap)] w-full">
            <form
                ...
                className="space-y-[var(--admin-section-gap)]"
            >
                <AdminSectionCard
                ...
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-1">
                            <label className="block text-small font-[var(--fw-bold)] text-[var(--text-muted)] uppercase tracking-wider mb-2">
                                {t("institution_name")}
                            </label>
                            <input
                        ...
                        <div>
                            <label className="block text-small font-[var(--fw-bold)] text-[var(--text-muted)] uppercase tracking-wider mb-3">
                                {t("language_setting")}
                            </label>
                            <div className="flex gap-4 p-1">
                                <button
                                    type="button"
                                    onClick={() =>
                                        updateForm({ language: "he" })}
                                    className={`flex-1 justify-center flex items-center gap-3 px-4 py-2 rounded-[var(--radius-main)] border transition-all ${
                                ...
                                <button
                                    type="button"
                                    onClick={() =>
                                        updateForm({ language: "en" })}
                                    className={`flex-1 justify-center flex items-center gap-3 px-4 py-2 rounded-[var(--radius-main)] border transition-all ${
                                        formData.language === "en"
                                            ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-900 dark:text-indigo-400"
                                            : "border-[var(--border-main)] hover:bg-[var(--bg-hover)] text-[var(--text-main)]"
                                    }`}
                                >
                                    <div
                        ...
                        <div className="md:col-span-2 pt-6 border-t border-[var(--border-subtle)]">
                            ...
                            {uploadError && (
                                <p className="text-red-500 text-[var(--fs-xs)] mt-2 font-[var(--fw-bold)]">
                                    {uploadError}
                                </p>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {formData.background_music_url && (
                                            <button
                                                ...
                                                className={`px-4 rounded-[var(--radius-main)] border transition-all flex items-center justify-center gap-2 ${
                                                    isPreviewPlaying
                                                        ? "bg-amber-500 border-amber-600 text-white shadow-lg scale-105"
                                                        : "bg-[var(--bg-surface)] border-[var(--border-main)] text-[var(--text-main)] hover:bg-[var(--bg-hover)]"
                                                }`}
                                                title={isPreviewPlaying
                                                    ? t("pause_preview" as any)
                                                    : t("play_preview" as any)}
                                            >
                                                {isPreviewPlaying
                                                    ? (
                                                ...
                                                <span className="text-[var(--fs-xs)] font-[var(--fw-bold)] uppercase hidden sm:inline">
                                                    {isPreviewPlaying
                                                        ? t("pause" as any)
                                                        : t("preview" as any)}
                                                </span>
                                ...
                                <div className="flex items-center justify-between p-4 bg-[var(--bg-surface)] rounded-[var(--radius-main)] border border-[var(--border-subtle)] shadow-inner">
                                    <span className="text-[var(--fs-base)] font-[var(--fw-bold)] text-[var(--text-main)]">
                                        {t("playback_mode")}
                                    </span>
                                    <div className="flex bg-[var(--bg-card)] p-1 rounded-[var(--radius-main)] border border-[var(--border-subtle)] shadow-sm">
                                        <button
                                            ...
                                            className={`flex-1 px-4 py-1.5 rounded-md text-small font-[var(--fw-bold)] uppercase tracking-wider transition-all ${
                                                formData
                                                        .background_music_mode ===
                                                        "loop"
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center px-1">
                                        <label className="block text-small font-[var(--fw-bold)] text-[var(--text-muted)] uppercase tracking-wider">
                                            {t("initial_volume")}
                                        </label>
                                        <span className="text-small font-[var(--fw-bold)] text-indigo-600">
                                            {formData.background_music_volume ||
                                                50}%
                                        </span>
                                    </div>
                ...
                >
                    ...
                    <div className="space-y-4 pt-6 mt-6 border-t border-[var(--border-subtle)]">
                        ...
                        <div className="flex flex-wrap gap-3">
                            {(formData.score_presets || []).map((
                                preset,
                                idx,
                            ) => (
                                <div
                                    key={idx}
                                    className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-[var(--radius-main)] px-4 py-2 flex items-center gap-3 shadow-sm hover:border-indigo-500 transition-colors group"
                                >
                                    <span className="text-body font-[var(--fw-bold)] text-[var(--text-main)]">
                                        {preset.label}
                                    </span>
                    ...
                    <div className="space-y-4 pt-6 mt-6 border-t border-[var(--border-subtle)]">
                        <h4 className="block text-small font-[var(--fw-bold)] text-[var(--text-muted)] uppercase tracking-wider mb-2">
                            {t("add_new_button")}
                        </h4>
                        <div className="flex flex-col sm:flex-row gap-4 p-6 bg-[var(--bg-surface)] rounded-[var(--radius-main)] border border-[var(--border-main)] shadow-inner">
                            <div className="flex-1 space-y-1">
                                <label className="text-small font-[var(--fw-bold)] uppercase text-[var(--text-muted)]">
                                    {t("button_label")}
                                </label>
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 rounded-[var(--radius-main)] border border-[var(--border-main)] bg-[var(--bg-input)] text-[var(--text-main)] font-[var(--fw-bold)] focus:ring-2 focus:ring-indigo-500 transition-all outline-none text-body shadow-sm"
                                    placeholder={t("button_label_placeholder")}
                                />
                            </div>
                            <div className="sm:w-32 space-y-1">
                                <label className="text-small font-[var(--fw-bold)] uppercase text-[var(--text-muted)] text-center block">
                                    {t("points")}
                                </label>
                                <input
                                    type="text"
                        ...
                        >
                            <div className="flex-1 px-2">
                                {message
                                    ? (
                                        <span
                                            className={`text-[var(--fs-base)] font-[var(--fw-bold)] ${
                                    ...
                                    : (
                                        <span className="text-[var(--fs-base)] font-[var(--fw-bold)]">
                                            {t("unsaved_changes")}
                                        </span>
                                    )}
                            </div>
