    ...
    return (
        <div className="min-h-full flex flex-col bg-background text-foreground overflow-x-hidden" dir={dir}>
            ...
            <main className="relative z-10 w-full max-w-2xl mx-auto px-6 pt-8 md:px-8 md:pt-12 flex-1 pb-16 rtl:text-right ltr:text-left">
                ...
                {/* ── HERO ── */}
                <MotionDiv initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
                ...
                <MotionDiv initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mb-10">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            { bad: 'מנצח אחד בטבלה', good: 'כל תלמיד מתחרה מול הגרסה הקודמת של עצמו' },
                            { bad: 'מי שמאחור - מאבד עניין', good: 'כל שיפור, קטן כגדול, מקבל במה ונחגג' },
                        ].map((row, i) => (
                            <React.Fragment key={i}>
                                <div className="bg-red-50 dark:bg-red-950/20 border border-red-200/50 dark:border-red-800/30 rounded-xl p-3 flex items-start gap-2">
                                    <span className="text-red-400 mt-0.5 shrink-0">✕</span>
                                    <span className="text-sm text-muted-foreground">{row.bad}</span>
                                </div>
                                <div className="bg-green-50 dark:bg-green-950/20 border border-green-200/50 dark:border-green-800/30 rounded-xl p-3 flex items-start gap-2">
                ...
                <MotionDiv initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mb-10">
                    ...
                    <div className="space-y-3">
                        {[
                            ...
                            {
                                emoji: '🎯',
                                title: 'מד היעד - הפאזל המוסדי',
                                text: 'במקום פרס למנצח בודד, היעד המשותף מוצג כתמונה מוסתרת. כל נקודה שנצברת חושפת עוד חלק מהפאזל. הניצחון שייך לכולם יחד.',
                            },
                            {
                                emoji: '🎡',
                                title: 'גלגל המזל - כולם נשארים במשחק',
                                text: 'הגרלות שבהן כל נקודה שווה כרטיס נוסף, או חלוקה שוויונית לחלוטין. המסך מסתובב, המוזיקה עולה, והמתח מורגש בכל הכיתה.',
                            },
                        ].map(tool => (
                ...
                <MotionDiv initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="bg-gradient-to-br from-green-600 to-emerald-700 rounded-2xl p-6 md:p-8 shadow-xl text-white mb-8">
                    ...
                    <p className="text-white/70 text-sm mb-5">אשמח להציג הדגמה קצרה ופוקחת עיניים, ללא כל התחייבות.</p>
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <a
                            ...
                            className="flex items-center justify-center gap-2 bg-white text-green-700 hover:bg-green-50 rounded-xl px-5 py-3 font-black text-sm transition-colors flex-1 shadow"
                        >
