...
const shoutoutMessage = useMemo(() => {
        ...
        const templates = [
            t('groups_on_fire', { names }),
            t('groups_leading', { names }),
            t('groups_deciding', { names }),
            t('groups_advancing', { names }),
            t('groups_winning', { names })
        ];
...
{/* 1. Centered Image (Top) - 65% Height */}
                <div className="flex flex-col items-center justify-center h-[65%]">
                    <div className="relative aspect-square group rounded-[var(--radius-container)] overflow-hidden border-2 border-white/20" style={{ height: 'min(100%, clamp(160px, 20vw, 380px))', clipPath: 'inset(0 round 1rem)', transform: 'translateZ(0)' }}>
